...
def dequantize_weight(mod: nn.Module, dtype: torch.dtype | None = None) -> torch.Tensor:
    ...
    q = unpack_from_int32(mod.weight_packed, NUM_BITS, shape)  # int8, [out, in]
...
def _streaming_forward(mod: nn.Module):
    """Dequantize -> matmul -> free. Never materialises the full dense model."""

    def forward(x):
        w = dequantize_weight(mod, dtype=x.dtype)
        return F.linear(x, w, getattr(mod, "bias", None))

    return forward


def patch_packed_linears(model: nn.Module, verbose: bool = True) -> int:
        ...
        if hasattr(mod, "weight_packed") and hasattr(mod, "weight_scale"):
            if hasattr(mod, "_old_forward"):
                ...
                try:
                    ...
                    pass
            mod.forward = _streaming_forward(mod)
            n += 1
    if verbose:
        print(f"patched {n} packed Linear modules with on-the-fly dequantization")
    return n
